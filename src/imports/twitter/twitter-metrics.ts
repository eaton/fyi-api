...
export class TwitterMetrics extends BaseImport<TwitterMetricsRow[]> {
  ...
  async loadCache(): Promise<void> {
    this.cacheData = await this.files.readCache('metrics.json') as TwitterMetricsRow[];
  ...
  async fillCache(): Promise<TwitterMetricsRow[]> {
    const metrics = await this.files.findInput('daily_tweet_activity_metrics_*.csv');
    ...
    this.log(`Cached ${metrics.length} files (covering ${this.cacheData?.length || 0} days)`);
