...
export class Instagram extends BaseImport<InstagramCache> {
  ...
  async fillCache(): Promise<InstagramCache> {
    ...
    for (const file of files) {
      ...
      for (const incoming of raw) {
        const post: IGCachedPost = {
          ...
          date: incoming.creation_timestamp
            ? new Date(incoming.creation_timestamp * 1000).toISOString()
            : undefined,
    ...
    const profileFiles = [
      'login_and_account_creation/signup_information.json',
    ...
    this.log(
      `Cached ${cache.posts.length} posts by ${cache.profile['Username']} with ${mediaCount} media items`
    );
  ...
  async savePosts(cache: InstagramCache): Promise<void> {
    for (const post of cache.posts) {
      await this.db
        .collection('instagram_post')
        .save({ _key: post.id, ...post });
    }
  }
}
