...
export interface DrupalOrgOptions
...
export class DrupalOrg extends BaseImport<DrupalOrgCache> {
  ...
  async fillCache() {
    const uid = is.number(this.options.userId)
      ? this.options.userId
      : is.numericString(this.options.userId)
      ? Number.parseInt(this.options.userId)
      : 0;
    ...
    const crawler = new CheerioCrawler({
      ...
      requestHandler: async (context) => {
        ...
        if (pageType === 'profile') {
          ...
          this.log(
            `Cached drupal.org account info for ${cache.profile.handle}`
          );
        } else if (pageType === 'tracker') {
          if (this.options.nodes) {
            await context.enqueueLinks({
              globs: ['**/user/*/track*', '**/(project,forum)/**']
            });
          }
...
function getPageType(url: string) {
  ...
  if (parsed.path[0] === 'project' && parsed.path[2] === 'issues')
    return 'issue';
  if (parsed.path[0] === 'project' && parsed.path[2] === 'releases')
    return 'release';
...
export async function extractProfile(html: string) {
  return Html.extract(html, {
    ...
    companies:
      'div.field-name-companies-worked-for div.field-items | text | split:,',
    industries:
      'div.field-name-field-industries-worked-in div.field-items | text | split:,',
    expertise: [
      {
    ...
    .then((profile) => {
      if (profile.date)
        profile.date = Dates.fromOffset(profile.date).toISOString();
      if (profile.name) profile.name = profile.name.split('(').shift()?.trim();
      if (profile.socialLinks)
        profile.socialLinks = profile.socialLinks.map(
          (v) => (v as unknown as { value: string }).value
        );
      if (profile.events)
        profile.events = profile.events.map(
          (v) => (v as unknown as { value: string }).value
        );
      if (profile.expertise)
        profile.expertise = profile.expertise.map(
          (v) => (v as unknown as { value: string }).value
        );
      return profile;
    });
...
export async function extractTrackerActivity(html: string, targetUid?: number) {
  return Html.extract(html, {
    ...
    .then((activity) =>
      activity.filter((a) => (targetUid ? a.uid === targetUid : true))
    );
}
...
export async function extractIssue(html: string, userId?: number) {
  return Html.extract(html, {
    ...
    status:
      '#block-project-issue-issue-metadata div.field-name-field-issue-status div.field-item',
    author:
      '#block-project-issue-issue-metadata div.field-name-project-issue-created-by a | attr:href | split:/ | pop',
    ...
    priority:
      '#block-project-issue-issue-metadata div.field-name-field-issue-priority div.field-item',
    followers:
      'span.flag-project-issue-follow span.flag-tracker-count | split | shift | trim',
...
export async function extractProject(html: string) {
  return Html.extract(html, {
    ...
    usage:
      'ul.project-info li:nth-child(2) a strong | replace:, | parseAs:int | default:0',
    creator: 'ul.project-info li:nth-child(3) a',
    date: 'ul.project-info li:nth-child(3) time:nth-child(2) | parseAs:date',
    favorites:
      '#block-drupalorg-project-follow a.log-in-to-star | text | split | shift | replace:, | parseAs:int | default:0'
